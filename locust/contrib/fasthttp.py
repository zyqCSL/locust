...
class FastHttpSession(object):
    auth_header: str = None # type: ignore
    ...
    def _send_request_safe_mode(self, method, url, **kwargs):
        ...
        try:
            ...
            r = getattr(e, "response", ErrorResponse())
            r.error = e
            return r
    ...
    def request(self, method: str, path: str, name: str=None, data: str=None, catch_response: bool=False, stream: bool=False,
        ...
        if stream:
            request_meta["content_size"] = int(response.headers.get("content-length", 0))
        else:
        ...
        request_meta["response_time"] = int((default_timer() - float(request_meta["start_time"])) * 1000)
...
class FastHttpLocust(Locust):
    ...
    It's significantly faster, but not quite as capable.
    """
